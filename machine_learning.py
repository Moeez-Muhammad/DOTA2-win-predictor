...
with open('dt_scores', 'w+') as dt_file:
	dt_file.write("Accuracy: %0.2f (+/- %0.2f)" % (dt_scores.mean(), dt_scores.std() * 2))
...
with open('rf_scores', 'w+') as rf_file:
	rf_file.write("Accuracy: %0.2f (+/- %0.2f)" % (rf_scores.mean(), rf_scores.std() * 2))
...
with open('svm_scores', 'w+') as svm_file:
	svm_file.write("Accuracy: %0.2f (+/- %0.2f)" % (svm_scores.mean(), svm_scores.std() * 2))
dump(svm, 'svm.joblib')
...
with open('nn_scores', 'w+') as nn_file:
	nn_file.write("Accuracy: %0.2f (+/- %0.2f)" % (nn_scores.mean(), nn_scores.std() * 2))
...
with open('lr_scores', 'w+') as lr_file:
	lr_file.write("Accuracy: %0.2f (+/- %0.2f)" % (lr_scores.mean(), lr_scores.std() * 2))
